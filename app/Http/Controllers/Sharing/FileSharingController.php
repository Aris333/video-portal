...
class FileSharingController extends Controller
{
     ...
     * @return array
     */
    public function authProjectShared(Request $request, $token)
    {
        // Get sharing record
        $shared = Share::where(DB::raw('BINARY `token`'), $token)->firstOrFail();
    ...
    public function file_public_Token($token)
    {
        // Get sharing record
        $shared = Share::where(DB::raw('BINARY `token`'), $token)->firstOrFail();
     ...
     * @return mixed
     */
    public function file_detail_public($token)
    {
        // Get sharing record
        $shared = Share::where(DB::raw('BINARY `token`'), $token)->firstOrFail();
    ...
    public function file_detail_reveiw($id)
    {
        ...
        // Return record
        if ($file) {
            return response()->json( [ 'message'=>'File Found Successfully','status' =>201,'data'=> $file]);
        }else {
            return response()->json('File not saved Successfully', 401, []);
        }
    }
