 ...
 *  @method static \Illuminate\Database\Eloquent\Builder|\App\FileManagerFile whereThum_Img($value)
...
class FileManagerFile extends Model
{
    ...
    {
        // Get thumbnail from external storage
        if ($this->attributes['thumb_img'] && is_storage_driver(['s3', 'spaces', 'wasabi', 'backblaze'])) {

            return Storage::temporaryUrl('file-manager/' . $this->attributes['thumb_img'], now()->addHour());
        }

        // Get thumbnail from local storage
        if ($this->attributes['thumb_img']) {

            // Thumbnail route
            $route = route('thumbImg', ['name' => $this->attributes['thumb_img']]);
    ...
    {
        return $this->belongsTo(self::class,'parent_id','unique_id');

    }

    public function versions()
    {
        return $this->hasMany(FileVersion::class,'section_id','parent_id');
    }
