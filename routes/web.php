...
Route::get('/getImagesfolders/{id}', 'FileBrowser\BrowseController@getImagesDownload');

// Get public thumbnails and files
Route::get('/thumbnail/{name}/public/{token}', 'FileAccessController@get_thumbnail_public');
Route::get('/thumbImg/{name}/public/{token}', 'FileAccessController@get_thumbImg_public');

//Route::get('/annotationImg/{name}/public/{token}', 'FileAccessController@get_AnnotationImg');
...
Route::group(['middleware' => ['auth:api', 'auth.shared', 'auth.master', 'scope:master,editor,visitor']], function () {
    ...
    Route::get('/thumbImg/{name}', 'FileAccessController@get_thumbImg')->name('thumbImg');
    Route::get('/annotationImg/{name}','FileAccessController@get_AnnotationImg')->name('annotationImg');
    Route::get('/file/{name}', 'FileAccessController@get_file')->name('file');
});
Route::get('/file/{name}', 'FileAccessController@get_file_presentation')->name('file');
// Get user invoice
