<template>
  <div class="navigation-panel">
    ...
    <div class="name-wrapper">
      ...
      <div
        v-if="$route.name != 'ComparisonMediaView' && myFileInfo"
        class="name-count-wrapper"
      >
    ...
    >
      ...
      <div
        class="dropdown"
        style="position: absolute; padding: 4px; margin-left: -304px; top: -1px"
        v-if="version_Options.length > 1"
      >
        <button
          class="btn btn-secondary dropdown-toggle"
          style="background-color: #00bc7e"
          type="button"
          ...
          aria-expanded="false"
        >
          {{ "V" + version_Options.length }}
        </button>
        <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
          <a
            class="dropdown-item text-justify"
            v-for="(data, index) in version_Options"
            href="#"
            :key="index"
          >
            {{ "v" + index++ }}
            <span
              style="cursor: pointer; color: #00bc7e"
              @click="
                updateRevsion(data, 'show');
                showShare = !showShare;
              "
              >{{ data.name }}</span
            >
          ...
          <a class="dropdown-item text-center" href="#">
            <span
              style="cursor: pointer; color: #00bc7e"
              @click="
                updateRevsion(data, 'compare');
                showShare = !showShare;
              "
              >Compare Version <i class="fas fa-window-restore"></i
            ></span>
          </a>
        </div>
      </div>
      ...
      >
        ...
        <div class="col-sm-8" style="position: absolute; top: -19px">
          ...
          >
            ...
            <template slot="option" slot-scope="data">
              <div class="d-center">
                <p v-if="data.label === 'Needs Review'">
                  <span class="dot" style="background-color: #003bb3"></span
                  ><span class="dot-txt">{{ data.label }}</span>
                </p>
                <p v-if="data.label === 'In Progress'">
                  <span
                    class="dot"
                ...
                <p v-if="data.label === 'Approved'">
                  <span class="dot" style="background-color: greenyellow"></span
                  ><span class="dot-txt">{{ data.label }}</span>
                </p>
                <p v-if="data.label === 'No Status'">
                  <span class="dot" style="background-color: #0077a2"></span
                  ><span class="dot-txt"> {{ data.label }}</span>
                </p>
              </div>
            </template>
    ...
    >
      <div class="navigation-tool-wrapper">
        ...
        />
        <ToolbarButton
          v-if="this.myFileInfo.type === 'image'"
          source="print"
...
export default {
  ...
  data() {
    return {
      options: ["Needs Review", "In Progress", "Approved", "No Status"],
      status: "",
  ...
  methods: {
    ...
    updateRevsion: function (data, vers) {
      if (vers == "show") {
        const encryptedText = btoa(data.id);
