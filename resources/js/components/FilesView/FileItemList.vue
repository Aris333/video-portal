	...
	<div class="file-wrapper" oncontextmenu="return false;"  @click.stop="clickedItem" @dblclick="goToItem" spellcheck="false">
		...
		>
			...
			<div class="icon-item"  >
				...
				<!--Image thumbnail-->
        <div class="">
          <img loading="lazy"  class="image" v-if="isImage" :src="data.thumbnail" :alt="data.name" />
          <i v-if="isImage"
				...
				<b ref="name" @input="renameItem" :contenteditable="canEditName" class="name">
					...
					<i v-if="isFile " 
					 style="
							...
							width:19px;
					    " 
						class="fas fa-code-branch versionSymbol"
						 @click="openModel(data)"
					></i>
				</b>
...
<!--					<span v-if="!isFolder && data.status ==='Approved'" class="item-size">{{ data.filesize }}, {{ timeStamp }}  , <strong  v-if="data.status !='No Status'" >Status :  {{data.status !='No Status' ?data.status :'' }} <span class="dot" style="background-color: lightskyblue" ></span> </strong></span>-->
					...
					<span v-if="!isFolder && data.parent_id ==='0' && data.status ==='No Status'" class="item-size">{{ data.filesize }},
<!--            {{ timeStamp }} , -->
            <strong  v-if="data.status !='No Status'" >Status :  {{data.status  ?data.status :'' }}<span class="dot"  style="background-color: greenyellow"></span>  </strong>
            ...
            <strong> {{data.status}}</strong>
					...
					<!--Folder item counts-->
					<span v-if="isFolder" class="item-length"> {{ folderItems == 0 ? $t('folder.empty') : $tc('folder.item_counts', folderItems) }}
<!--            , {{ timeStamp }}-->
            ,<strong  v-if="isFolder =='false'    && data.status !='No Status'" >Status :  {{data.status !='No Status' ?data.status :'' }}</strong> </span>
				</div>
			</div>
			<!--Go Next icon-->
