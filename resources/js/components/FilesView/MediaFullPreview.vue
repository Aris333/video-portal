...
<!--  <vue-loading type="bars" color="#d9544e" :size="{ width: '50px', height: '50px' }"></vue-loading>-->
  ...
  <div class="media-full-preview" id="mediaPreview" v-if="this.isMedia && fileInfoDetail && isLoading ===false"  >
...
<!--													<time-ago :refresh="60" :datetime="data.created_at" locale="en" tooltip style="color:white;    background-color: #fff; margin-left:10px;"></time-ago>-->
                ...
                <div class="pt-2">
                  <div class="d-inline">
                    <span class="pt-5" style="color:#00BC7E" v-if="myFileInfo && myFileInfo.type === 'video'"  >{{data.video_time.slice(0,8)}}</span>
                ...
                <div class="pt-2" v-show="showEditByIndex === index" >
                  ...
                  <button class="btn-save white"  @click.prevent="updateComments(data,'','update'); showEditByIndex=''">Save</button>
                  <button class="btn-save white"  @click.prevent="showEditByIndex ='';">Cancel</button>
                </div>
                <!--              v-if="isReplying"-->
                <div class="pt-2"    v-show="showReplyByIndex === data.id "  >
                  ...
                  <button class="btn-save white"  @click.prevent="replyComment(data); showReplyByIndex ==='' ">Send</button>
                  <button class="btn-save white"  @click.prevent="showReplyByIndex =''">Cancel</button>
                </div>
...
<!--													<time-ago :refresh="60" :datetime="childData.created_at" locale="en" tooltip style="color:white;    background-color: #fff; margin-left:10px;"></time-ago>-->
                   ...
                   <i class="fas fa-thumbs-up" v-show="showLikeByIndex.index === index && childData.id"></i>
                  </span>
                      ...
                      <div class="text-white d-inline ml-2 float-right" v-if="childData.user.name === user.data.attributes.name">
                        ...
                        >
                          <i class="fal fa-pen text-muted"  style="  color:#00BC7E !important;"
                             data-toggle="collapse" :href="'#multiCollapseExample_'+index" role="button"
                             aria-expanded="false" aria-controls="multiCollapseExample1"
                             @click.prevent="editComments(childData,index); showChildEditByIndex=childData.id"></i>
                        </button>
                        <!---delete btn -->
                        <button class="btn-save"
                    ...
                    <!--    comment-->
                    <div class="pt-2" v-show="showChildEditByIndex ===childData.id" >
                      <div class="d-inline">
                        <input type="text" class="form-control" v-model="edit_comment"  >
                      </div>
                      <button class="btn-save white"   @click.prevent="updateComments(childData,'','update'); showChildEditByIndex=''">Save</button>
                      <button class="btn-save white" @click.prevent="showChildEditByIndex ='';">Cancel</button>
                    </div>
                    <!--              v-if="isReplying"-->
                    <div class="pt-2"    v-show="showChildReplyByIndex ===childData.id  && childData.parent_id ==data.id"  >
                      ...
                      <button class="btn-save white"   @click.prevent="replyComment(childData); showChildReplyByIndex ==='' ">Send</button>
                      <button class="btn-save white"  @click.prevent="showChildReplyByIndex =''">Cancel</button>
                    </div>
                  </div>
                </div>
              </div>
...
export default {
	...
	components: { vuescroll,ToolbarButton,Editor ,Spinner, VueSlider,Painting, FrameVideo,EmojiPicker,Tabs ,VueContentLoading,moment },
	computed: {
...
//  $('.vs__search').css( 'padding-left','45px')
  ...
  $('.p-video-player__media').bind('contextmenu',function() { return false; });
   ...
   this.commentLoading = true
     let data =  { file_id:this.currentFile.id, sortBy:this.commentSortBy ? this.commentSortBy : ''}
    ...
    drawImage: async function(data)
    {
     ...
     let ctx = this.$refs.paintable.ctx
      ...
      //  changes after the error in 11-3-2021
