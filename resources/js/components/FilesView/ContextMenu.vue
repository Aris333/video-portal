    ...
    <div :style="{ top: positionY + 'px', left: positionX + 'px' }" @click="closeAndResetContextMenu" class="contextmenu" v-show="isVisible || showFromPreview" ref="contextmenu" :class="{ 'filePreviewFixed' : showFromPreview}">
        ...
        " id="menu-list" class="menu-options">
            ...
            <ul class="menu-option-group" v-if="item">
                ...
                <li class="menu-option" @click="copyItem">
                    <div class="icon">
                        <copy-icon size="17"></copy-icon>
                    </div>
                    <div class="text-label">
                      {{ $t('context_menu.copy') }}
                    </div>
...
export default {
    ...
    methods: {
        ...
        copyItem() {
            // Open move item popup
            events.$emit('popup:open', { name: 'copy', item: this.item })
        },
        shareItem() {
            if (this.item.shared) {
        ...
        downloadItem() {
            ...
            if(this.item.type == 'folder'){
                this.$store.dispatch('downloadFolderImages', this.item)
                // this.$downloadFile(
                //     this.item.file_url,
