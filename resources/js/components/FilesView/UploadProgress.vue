<template>
    <transition name="info-panel">
        <div v-if="uploadingFilesCount" class="upload-progress">
            ...
            <ProgressBar :progress="uploadingFileProgress" :time="uploadingFileTime"/>
            <div class="progress-title">
                <span v-if="!isProcessingFile && uploadingFilesCount.total === 1">
                    {{ uploadingFileTime}}
                </span>
            ...
            <!-- <span>Remaining Time: {{this.rem_time+' '+this.unit}}</span> -->
        </div>
    </transition>
</template>
...
<script>
    ...
    export default {
        ...
        watch: {
            uploadingFileProgress: function (val) {
                var rem_per = 100 - this.uploadingFileProgress;
                this.rem_time = (rem_per / 10) * 1;
                // console.log(this.rem_time);
