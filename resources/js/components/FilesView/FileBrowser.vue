<template>
    ...
    <div class="file-content" id='file_content' :class="{ 'is-offset': uploadingFilesCount, 'is-dragging': isDragging }"
         @dragover.prevent
...
<script>
    ...
    export default {
        ...
        computed: {
            ...
            versionList() {
                return this.versionListLength == 0 ? true : false
            },
        },
        data() {
            return {
      ...
      mounted(){
            ...
            dragFinish(data, event) {
              ...
              console.log('event.dataTransfer',event.dataTransfer.items.length )
                if (event.dataTransfer.items.length == 0 || event.dataTransfer.items.length == 3) {
                    ...
                    {
                      // return
                      // custom code done at 19-04-2021    //&& this.versionListLength < 0


                      if(this.draggingId.id != data.id   ){
                        if(this.versionCompActive ==true)
                        {
                          alert(1)
                        }else{
                      ...
                      // else{
                      //     // alert(1)
                      //   console.log('version_jhkjh')
                      //   // this.$store.dispatch('changeVersionParent', [this.draggingId, data])
                      // }
                    }
        ...
        updated(){
                $('#file_content').css('position','none');
        },
        created() {
