<template>
    <PopupWrapper name="share-review-create">
        <!--Title-->
        <PopupHeader :title="$t('popup_share_review_create.title', {item: itemTypeTitle})" icon="share" />
    ...
    export default {
            ...
            async submitShareOptions() {
                ...
                axios
                    ...
                    .then(response => {
                      ...
                      var res =    link.replace('shared','sharedReview')
                      var complete_url =res +'?q='+ btoa(this.pickedItem.id)
                      this.shareLink =  complete_url
                        // this.shareLink = response.data.data.attributes.link
                        this.isGeneratedShared = true
                        let attributes = response.data.data.attributes
                      attributes['link'] =this.shareLink

                        this.$store.commit('UPDATE_SHARED_ITEM', attributes)
                    })
                    .catch(error => {
        ...
        mounted() {
            ...
            events.$on('popup:open', args => {
                ...
                if (args.name !== 'share-review-create') return
