  ...
  <div class="">
    ...
    <div class="media-full-preview" id="mediaPreview" v-if="this.isMedia && fileInfoDetail"  >
      <div class="row"  >
        <div class="col-sm-9">
          ...
          <div class="file-wrapper-preview" >
            ...
            <div class="file-wrapper">
              ...
              <img v-if="fileInfoDetail.type === 'image' && currentFile.thumbnail" class="file" :class="{ 'file-shadow': !isMobileDevice }" id="image" src="601bab8e4217c.jpeg" />
              <div class="video-wrapper">
              </div>
              <!--<h1>{{isProcessingFile}}</h1> v-show="isLoading == false"    -->
              <div class="l-page" v-if="fileInfoDetail.type === 'video' && currentFile.file_url" >
                  ...
                  <div class="p-video-player__seekbar-wrap-progress" id="video-player-media">
                    <div class="progress"  style="height:25px;">
                      ...
                      >
                        ...
                        <div slot="popover" class="text-left">
                          <div>
                          ...
                          <div class="pt-2">
                            <div class="d-inline">
                              <span class="pt-5" style="color:#00BC7E">{{  data.video_time ? data.video_time.slice(0,8) : ''}}</span>
                            </div>
                    ...
                    <div class="row">
                      <div class="col-sm-12">
                        <div class="d-inline float-left" >
                          <a v-if="isPlaying" @click="playOrPause">
                      ...
                      <div class="card card-white post" style="border-radius: 11px;">
                        ...
                        <div class="container">
                          ...
                          <div class="first">
                            ...
                            <button class="dropbtn float-right" style=" width:36px; border-radius:23px;"  @click="isActive =!isActive;showbtns=!showbtns"><i class="fas fa-palette" style="color: #00BC7E !important;"  ></i></button>
                            <div class="d-inline float-right paintTool" v-if="isActive">
        ...
        <div class="col-sm-3 pl-2 dynamic-theme-color" >
          <div class="mt-3">
            <div v-if="currentFile.description"  class="p-2">
            ...
            <a class="text-white pt-2" @click="isDescription = false" v-if="isDescription == true">
              ...
              <span class="pl-2 ">Add Description</span>
            </a>
          ...
          <div class="row mt-3">
            <div class="col-sm-12">
              <nav class="tabs">
                <button class="tabs__item" style="color: black" v-bind:class="{ tabs__item_active: currentTab }" @click="handleClick(true)"> {{allComments == undefined ? 0 : this.allComments.length}} Comments </button>
                <!-- active tab -->
                <button class="tabs__item" style="color: black"   v-bind:class="{ tabs__item_active: !currentTab }" @click="handleClick(false)"> File Information</button>
              </nav>
            </div>
          </div>
...
<!--													<time-ago :refresh="60" :datetime="data.created_at" locale="en" tooltip style="color:white;    background-color: #fff; margin-left:10px;"></time-ago>-->
                ...
                <div class="pt-2" >
                  <div class="d-inline"      @click="showReplyByIndex = index; reply='';parentId=data.id">
                  ...
                  <div class="text-white d-inline ml-2 float-right">
                    <button class="btn-save mr-1">
                      <i class="fal fa-pen text-muted"   style="  color:#00BC7E !important;"
                         ...
                         @click.prevent="editComments(data,index); showEditByIndex=index"></i>
                    </button>
                    <!---delete btn -->
                ...
                <!--              v-if="isReplying"-->
                <div class="pt-2"    v-show="showReplyByIndex === index && data "  >
                  <div class="d-inline">
...
<!--													<time-ago :refresh="60" :datetime="childData.created_at" locale="en" tooltip style="color:white;    background-color: #fff; margin-left:10px;"></time-ago>-->
                    ...
                    <div class="pt-2" >
                      <div class="d-inline"      @click="showChildReplyByIndex = index; reply=''; ">
                        <span class="pt-5 text-muted">Reply</span>
                      </div>
                      <div class="text-white d-inline ml-2" >
                   ...
                   <i class="fas fa-thumbs-up" v-show="showLikeByIndex.index === index"></i>
                    ...
                    <!--              v-if="isReplying"-->
                    <div class="pt-2"    v-show="showChildReplyByIndex ===index && childData.parent_id ==data.id"  >
                      <div class="d-inline">
                        <input type="text" class="form-control" v-model="reply" >
                      </div>
...
export default {
  ...
  computed: {
    ...mapGetters(['fileInfoDetail','annotation','isProcessingFile','updateFps', 'data','user','allComments']),
    ...
    getTotalFrame: function() {
      ...
      // console.log('currentVideoFps',this.currentFile.fps)
      // console.log('.currentVideoTime',this.getDuration)
...
//    this.$refs.paintable.clearCanvas()
  ...
  async mounted() {
    ...
    $('#vs2__combobox').css( 'background-color','lightgray')
...
//  $('.vs__search').css( 'padding-left','45px')
    let backcanvas= $('.canvas back active').attr('width');

    var width= $('video#videoElement').clientWidth;
    $('.back').width = 100%  // eslint-disable-line
        $('.readBy').hover(function() {
          $(this).closest('.row').find('.text-muted').hide();
        });
    $(".canvas back active").attr('id','editorId');

    $('.back').height =100%

        $(document).ready(function() {
          $(".canvas back active").mousemove(function(event){
    ...
    $('.p-video-player__media').bind('contextmenu',function() { return false; });
  ...
  methods: {
    ...
    getComments: async function() {
      ...
      let data =  { file_id:this.$route.query.q, sortBy:this.commentSortBy ? this.commentSortBy : ''}
      this.$store.state.allComments =null
      await  this.$store.dispatch('getComment',data)
      let vm = this;
    ...
    drawImage: async function(data)
    {
      ...
      img.src =  'annotationImg/'+data.annotation; // eslint-disable-line

    },

    getImage:  function() {
      this.$store.dispatch('getImage',
          {file_id:this.currentFile.id, user_id:this.user.data.id ? this.user.data.id : '' ,currentTime:this.media.currentTime})
    },
    ...
    likeComment:function (data,like=''){
      ...
      this.$store.dispatch('likeComment', formData)
    },
...
//
//       //Variables
//       var scribble_canvasx = $('.canvas back active').offset().left;
//       var scribble_canvasy = $('.canvas back active').offset().top;
//       var scribble_last_mousex = scribble_last_mousey = 0;
//       var scribble_mousex = scribble_mousey = 0;
//       var scribble_mousedown = false;
//
// //Mousedown
//       $('.canvas back active').on('mousedown', function(e) {
...
//
// //Mouseup
//       $('.canvas back active').on('mouseup', function(e) {
//         scribble_mousedown = false;
//       });
//
// //Mousemove
//       $('.canvas back active').on('mousemove', function(e) {
...
//           ctx.clearRect(0,0,this.$refs.paintable.canvas.width,this.$refs.paintable.canvas.height); //clear canvas
//           //Save
//           ctx.save();
//           ctx.beginPath();
//           //Dynamic scaling
