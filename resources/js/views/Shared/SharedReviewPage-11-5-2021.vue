...
<!--        <div class="">-->
<!--          <p v-if="version_show== true">hello</p>-->
<!--        </div>-->
<!--      </transition>-->
      ...
      <div class="media-full-preview"  v-show="isLoading ===false" >
        ...
        <div class="row" id="testing" >
          <div class="col-sm-9">
            <div class="file-wrapper-preview" v-show="isLoading ===false" >
              <div class="file-wrapper">
                <audio class="file audio" :class="{ 'file-shadow': !isMobileDevice }" v-if="myFileInfo && myFileInfo.type == 'audio'" :src="currentFile.file_url" controlsList="nodownload" controls></audio>
                ...
                <!-- Pic Paintable  -->
                <div class="l-page" v-if="myFileInfo && myFileInfo.type === 'image' && currentFile.file_url" >
                  <div class="p-video-player" id="video-player">
                    ...
                    <paintable
                        ...
                        :color="color"
                        ref="paintable"
                        class="paint col-sm-12 "
                        @toggle-paintable="toggledPaintable"
                    >
                      <div class="image-for-pic">
                        <img
                            id="paintImage"
                    ...
                    <div class="container "  style="margin-top: 15px">
                      <div class="row">
                        <div class="col-12">
                          <div class="card card-white post" style="border-radius: 11px;">
                            <div class="post-heading" style="margin-top: 10px">
                              <div class="float-left image">
                                <img :src="user.data.attributes.avatar ? user.data.attributes.avatar :'http://bootdey.com/img/Content/user_1.jpg'" class="img-circle avatar" style="border-radius: 20px; height: 40px!important; " alt="user profile image">
                              </div>
                              <div class="float-left meta">
                                <div class="title h5">
                                  <input type="text"  class="" style="width: 100%; margin-top:10px; color: #00BC7E !important; border:none;background:transparent;margin-left: 10px;margin-top: 3px;"
                                         placeholder="Comment" aria-label="userComment" aria-describedby="basic-addon1"  v-model="comment">
                                </div>
                            ...
                            <div class="container">
                              ...
                              <div class="first">
                                <transition-group name="slide-fade">
                                  <button v-if="showbtns"  class="dropbtn" key="1"  id="time_btn" style="margin-bottom: 2px;  color:black;borders-radius:11px; width: 98px" >
                                    <i class="fas fa-clock" style="color: #00BC7E !important;"  ></i><span style="color: #00BC7E !important;"> 00:00 </span>
                                    <input type="checkbox" v-model="selectedTime" class="">
                                  </button>

                                  <div key="2"  v-if="showbtns" class="pr-fio2 float-left" >
                                    <div class="relative" style="transform: scale(1, 1);">
                                      <div class=" float-right">

                                        <v-select :options="['Every One','Team Group']"  style="margin: 6px"   v-model="permission"    append-to-body :calculate-position="withPopper" />

                                      </div>
                                    </div>
                                  </div>
                                </transition-group>
                                <button class="dropbtn float-right"  id="" style="color:black;border-radius:23px; width: 78px;color: #00BC7E !important;" :disabled="comment ==''"  @click="submitComment" >Send</button>
                                <button class="dropbtn float-right" style=" width:36px; border-radius:23px;"  @click="isPaintActive =!isPaintActive;showbtns=!showbtns;  clearPaintable()"><i class="fas fa-palette" style="color: #00BC7E !important;"  ></i></button>
                                <div class="d-inline float-right paintTool" v-if="isPaintActive">
                                  ...
                                  <button class="draw-options"  @click="$refs.paintable.undoDrawingStep"><i style="  color:#00BC7E !important;" class="fas fa-undo"></i></button>
                                  ...
                                  <button class="draw-options"  @click="$refs.paintable.clearCanvas();$refs.paintable.saveCurrentCanvasState()"><i  style="  color:#00BC7E !important;" class="fas fa-backspace"></i></button>
                                  ...
                                  <button class="draw-options"  @click="useEraser = !useEraser">
                                    <i  v-show="useEraser" style="  color:#00BC7E !important;" class="fas fa-eraser"></i>
                                    <i  v-show="!useEraser" style="  color:#00BC7E !important;" class="fas fa-pencil-alt"></i>
                                  </button>
                                </div>
                              </div>
                            </div>
                ...
                <div class="l-page" v-if="myFileInfo && myFileInfo.type === 'video' && currentFile.file_url" >
                  <div class="p-video-player" id="video-player" >
                    <div class="custom-navigation">
                    </div>
                    ...
                    >
                      <img
                          id="paintImage"
                          style="position: absolute;display: none; height: 400px"
                          alt="Unsplash Background-Image"
                          :src="testImg"
                      />
                      <FrameVideo
                    ...
                    <div class="p-video-player__seekbar-wrap" id="video-player-media" @mousedown="grabSeekbar" @touchstart="grabSeekbar" @touchmove="moveSeekbar" @touchend="releaseSeekbar">
                      <div v-if="getProgressRate<=1"  class="p-video-player__seekbar-current" :style="{ transform: &quot;scaleX(&quot; + getProgressRate + &quot;)&quot; }"></div>
                      <div v-else class="p-video-player__seekbar-current" :style="{ transform: &quot;scaleX(&quot; + 1 + &quot;)&quot; }"></div>
                      <div class="p-video-player__seekbar-back"></div>
                    </div>

                    <div class="p-video-player__seekbar-wrap-progress" id="video-player-media">
                      <div class="progress"  style="height:25px;">
                        <div class="progress-bar color-white" role="progressbar" aria-valuenow="40" aria-valuemin="0" aria-valuemax="100" style="width: 100%;">
                        </div>
                        <v-popover v-for="(data,index) in allComments" :key="index" class="dot mt-1 text-white"
                                   :style="'left:'+(((parseFloat(data.text_edited_at)/duration)*100))+'%;'"
                                   trigger="hover" @click.native="seekbarCommentClick(data); drawImage(data)" placement="top"
                        >
                          <!-- This will be the popover target (for the events and position) -->
                          <!-- <button class="">Click me</button> -->
                          <span><small  class="text-white">{{ data.user_name ? data.user_name.slice(0,1) :''}}</small></span>
                          <!-- This will be the content of the popover -->
                          <div slot="popover" class="text-left" v-if="data">
                            <div>
                              <div class="d-inline ml-2 float-left text-left" style="color:#00BC7E">
                                {{data.user_name}}
                              </div>
                            </div><br>
                            <div class="pt-2">
                              <div class="d-inline">
                                <span class="pt-5"  v-if="myFileInfo && myFileInfo.type === 'video' "  style="color:#00BC7E">{{  data.video_time ? data.video_time.slice(0,8) : ''}}</span>
                              </div>
                              <div class=" d-inline ml-2">
                                <span class="text-muted" style="color:#00BC7E !important;">{{data.comment}}</span>
                              </div>
                            </div>
                          </div>
                        </v-popover>
                      ...
                      <div class="row">
                        <div class="col-sm-12">
                          ...
                          <a class="ml-5 float-left" @click="playLoop()" title="Loop">
                            <i class="fa fa-repeat" aria-hidden="true"   style="color: #00BC7E !important;" :class="{'iconColor': looping == true, 'iconColorWhite': looping == false }"></i>
                            <!-- {{ looping ? 'Loop' : 'Single' }} -->
                          </a>

                          <div class="dropdown show ml-3 float-left" style="display:inline-block;color: #00BC7E !important;" >
                            <a class="dropdown-toggle  "  href="#"    style="color: #00BC7E !important;"  role="button" id="dropdownMenuLink" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                              {{playback_speed}}x
                            </a>
                            <div class="dropdown-menu " style="color: #00BC7E !important;"  aria-labelledby="dropdownMenuLink">
                          ...
                          <vue-slider class="float-left mr-1"  v-on:change="SetVolume()" width="15%" v-model="val" :tooltip="'active'"/>

                          <div v-if="timeFormat=='Standard'" style="color: #00BC7E !important;"   class="p-video-player__time-wrap  ml-2 d-inline">
												<span class="p-video-player__time-current ">{{ getCurrentTime }}
												</span>/
                            <span class="p-video-player__time-all ">{{ getDuration }}</span>
                          </div>

                          <div v-if="timeFormat=='Timecode'" style="color: #00BC7E !important;"   class="p-video-player__time-wrap  ml-2 d-inline">
												<span class="p-video-player__time-current ">{{ getCurrentMilisecond }}
												</span>
                          </div>

                          <div v-if="timeFormat=='Frames'" style="color: #00BC7E !important;"  class="p-video-player__time-wrap  ml-2 d-inline">
												<span class="p-video-player__time-current"> {{ getFrames }}
												</span>/
                            <span style="color: #00BC7E !important;"   class="p-video-player__time-all">{{ getTotalFrame }}</span>
                          </div>
                          ...
                          <div class="dropdown show ml-3" style="display:inline-block;">
                            ...
                            <div class="dropdown-menu" aria-labelledby="dropdownMenuLink">
                              <a class="dropdown-item disabled" href="#" @click="playQualitySet(1080)"><strong  class="color-green">Time Format</strong></a>
                              <a class="dropdown-item" href="#" @click="setTimeFormat('Standard')"><strong class="color-green">Standard</strong></a>
                              <a class="dropdown-item" href="#" @click="setTimeFormat('Timecode')"><strong class="color-green">Timecode</strong></a>
                              <a class="dropdown-item" href="#" @click="setTimeFormat('Frames')"><strong class="color-green">Frames</strong></a>
                            </div>
                          </div>
                        ...
                        <div class="card card-white post" style=" border-radius: 11px;">
                          ...
                          <div class="container">
                            ...
                            <div class="first">
                              ...
                              <button class="dropbtn float-right" style="color:black;border-radius:23px; width: 78px;color: #00BC7E !important;" :disabled="comment ==''"  @click="submitComment" >Send</button>
                              <button class="dropbtn float-right" style=" width:36px; border-radius:23px;"  @click="isPaintActive =!isPaintActive;showbtns=!showbtns; clearPaintable()"><i class="fas fa-palette" style="color: #00BC7E !important;"  ></i></button>
                              <div class="d-inline float-right paintTool" v-if="isPaintActive">
          ...
          <div class="col-sm-3 pl-2 dynamic-theme-color"  v-show="isLoading ===false">
            <div class="mt-3">
              <div v-if="currentFile && currentFile.description"  class="p-2">
                <p>{{this.currentFile.description}}</p>
                <i class="fal fa-pen text-muted float-right"  style="  color:#00BC7E !important;"   @click="isDescription = !isDescription"  aria-hidden="true" ></i>
              </div>
              <a class="text-white pt-2" @click="isDescription = false" v-if="isDescription == true">
                <i class="fa fa-plus-square text-white text-decoration-none" aria-hidden="true"></i>
                <span class="pl-2 white">Add Description</span>
              </a>
              <div v-if="isDescription == false">
                <div class="p-2">
                  <textarea class="p-1" rows="5"  v-model="description" style="width:100%;border-radius:3px;">{{this.currentFile.description ? this.currentFile.description : this.description}}</textarea>
                </div>
                <div class="float-right">
                  <a class="text-muted mr-2" @click="isDescription = true">Cancel</a>
                  <a class="text-muted mr-2" @click="saveVideoDescription(); isDescription = true" >Save</a>
                </div>
              </div>
            </div>
            <div class="row mt-3">
              <div class="col-sm-12">
                <nav class="tabs">
                  <button class="tabs__item" v-bind:class="{ tabs__item_active: currentTab }" @click="handleClick(true)"> {{allComments == undefined ? 0 : this.allComments.length}} Comments </button>
                  <!-- active tab -->
                  <button class="tabs__item"   v-bind:class="{ tabs__item_active: !currentTab }" @click="handleClick(false)"> File Information</button>
                </nav>
              </div>
            </div>
            <div class="row" v-if="currentTab">
              <div class="col-sm-12">
                <div>
                  <div class="dropdown show ml-1" style="display:inline-block;">
                    <a class="dropdown-toggle btn-sm  text-white" href="#" role="button" id="dropdownMenuLink" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                      {{commentSortBy.replace(/^./, commentSortBy[0].toUpperCase())}}
                    </a>
                    <div class="dropdown-menu text-white" aria-labelledby="dropdownMenuLink">
                      <a class="dropdown-item disabled" href="#">Sort By</a>
                      <a class="dropdown-item" href="#" @click="sortComments('oldest')">Oldest</a>
                      <a class="dropdown-item" href="#" @click="sortComments('newest')">Newest</a>
                      <a class="dropdown-item" href="#" @click="sortComments('timecode')">Timecode</a>
                      <a class="dropdown-item" href="#" @click="sortComments('commenter')">Commenter</a>
                    </div>
                  </div>

                  <div class="float-right">
                    <small>
                      <a class="text-white m-2" href="#">Show<i class="fa fa-check-circle text-muted m-2"  style="  color:#00BC7E !important;"   aria-hidden="true"></i>
                      </a>
                    </small>
              ...
              <div class="col-sm-12">
                <div class="inner-addon left-addon">
                  <i class="fa fa-search" aria-hidden="true"   style="  color:#00BC7E !important;"  ></i>
                  <input type="text"  v-model="searchQuery"  placeholder="Search Comment" class="form-control" style="border-radius: 24px;height: 32px;" />
                </div>
              </div>
              <!-- class="overflow-auto" -->

              <div class="col-sm-12" >
                <Spinner v-if="commentLoading ==true" style="margin-top: 15px"></Spinner>
                <div v-show="commentLoading ==false"
                     id="1"  class="col-sm-12 mt-2 p-3  comment_color"  v-for="(data,index) in resultQuery" :key="index"
                     @mouseover="showByIndex = index;" @mouseout="showByIndex = null"
                >
                  <div>
                    <img style="width: 20px;height: 20px" :src="user.data.attributes.avatar ? user.data.attributes.avatar :'http://bootdey.com/img/Content/user_1.jpg'">
...
<!--													<time-ago :refresh="60" :datetime="data.created_at" locale="en" tooltip style="color:white;    background-color: #fff; margin-left:10px;"></time-ago>-->
                  ...
                  <div class="pt-2">
                    <div class="d-inline">
                      <span class="pt-5"  style="color:#00BC7E">{{data.video_time.slice(0,8)}}</span>
                      <span v-show="data.annotation !=''"    @click="seekbarCommentClick(data); drawImage(data)"><i  class="fas fa-paint-brush"></i></span>
                    </div>
                    <div class=" d-inline ml-2">
                  ...
                  <div class="pt-2" >
                    <div class="d-inline"      @click="showReplyByIndex = data.id; reply='';parentId=data.id">
                      <span class=" text-muted">Reply</span>
                    </div>
                    <div class="text-white d-inline ml-2" >
									<span class="text-muted"  @click.prevent="likeComment(data); showLikeByIndex.index = index; data.visible = !data.visible;  ">
                    <i class="far fa-thumbs-up text-muted"  v-show="data.visible ===!data.visible" ></i>
                  </span>
                      <span class="text-muted"     @click.prevent="data.visible = !data.visible; likeComment(data); "  >
                   <i class="fas fa-thumbs-up" v-show="showLikeByIndex.index === index"></i>
                  </span>
                    </div>
                    <div class="text-white d-inline ml-2  readBy"  >
                      <!--									<small class="text-muted"  v-show="showByIndex === index" >Read By 0 people</small>-->
                    </div>
                    <div class="text-white d-inline ml-2 float-right" v-if="data.user_name === user.data.attributes.name">
                      <button class="btn-save mr-1" >
                        <i class="fal fa-pen text-muted"   style="color:#00BC7E !important;"
                           data-toggle="collapse" :href="'#m' +
                            'ultiCollapseExample_'+index" role="button"
                           aria-expanded="false" aria-controls="multiCollapseExample1"
                           @click.prevent="editComments(data,index); showEditByIndex=index"
                        ></i>
                      </button>
                      <!---delete btn -->
                      <button class="btn-save"
                              @click.prevent="updateComments(data,'','delete'); data.visible =!data.visible">
                        <i class="fal fa-trash text-muted"  style="  color:#00BC7E !important;"></i>
                      </button>
                    </div>
                  </div>
                  <!--    comment-->
                  <div class="pt-2" v-show="showEditByIndex === index" >
                    <div class="d-inline">
                      <input type="text" class="form-control" v-model="edit_comment"  >
                    </div>
                    <button class="btn-save" style="color: #6c757d!important;"  @click.prevent="updateComments(data,'','update'); showEditByIndex=''">Save</button>
                    <button class="btn-save" style="color: #6c757d!important;" @click.prevent="showEditByIndex ='';">Cancel</button>
                  </div>
                  <!--              v-if="isReplying"-->
                  <div class="pt-2"    v-show="showReplyByIndex === data.id && data "  >
                    <div class="d-inline">
                      <input type="text" class="form-control" v-model="reply"  >
                    </div>
                    <button class="btn-save" style="color: #6c757d!important;"  @click.prevent="replyComment(data); showReplyByIndex ==='' ">Send</button>
                    <button class="btn-save" style="color: #6c757d!important;" @click.prevent="showReplyByIndex =''">Cancel</button>
                  </div>

                  <!--  Child  Relpies -->
                  <div class="col-sm-10 ml-3" v-show="data.replies">
                    <div
                        id="2"  class="col-sm-12 mt-2 p-3 comment_color " v-for="( childData,index) in data.replies" :key="index"
                        @mouseover="showChildByIndex = index;" @mouseout="showChildByIndex = null"
                    >
                      <div>
                        <div class="image-circle d-inline text-capitalize">
                          {{   childData.user.name ? childData.user.name.slice(0,1) :''}}
                        </div>
                        <!--                    <img v-show="childData.annotation ==''" :src="'../storage/app/images/annotation/'+childData.annotation">-->
                        <i class="fa fa-paint-brush"  v-show="childData.annotation" aria-hidden="true"></i>
                        <div class="d-inline ml-2" style="color:#00BC7E">
                          {{childData.user.name}}<span>
<!--													<time-ago :refresh="60" :datetime="childData.created_at" locale="en" tooltip style="color:white;    background-color: #fff; margin-left:10px;"></time-ago>-->
                        ...
                        <div class="float-right" v-if="activeId == childData.id">
                          <i class="fal fa-check-circle text-muted"></i>
                        </div>
                      </div>
                      <div class="pt-2">
                        <div class="d-inline">
                          <span class="pt-5" style="color:#5b53ff">{{childData.video_time.slice(0,8)}}</span>
                        </div>
                        <div class=" d-inline ml-2">
                          <span class="text-muted">{{childData.comment}}</span>
                        </div>
                      </div>
                      <div class="pt-2" >
                        <div class="d-inline"      @click="showChildReplyByIndex = childData.id; reply=''; ">
                          <span class="pt-5 text-muted">Reply</span>
                        </div>
                        <div class="text-white d-inline ml-2" >
									<span class="text-muted"  @click.prevent="likeComment(childData); showLikeByIndex.index = index; childData.visible = !childData.visible;  ">
                    <i class="far fa-thumbs-up text-muted"  v-show="childData.visible ===!childData.visible" ></i>
                  </span>
                          <span class="text-muted"     @click.prevent="childData.visible = !childData.visible; likeComment(childData); "  >
                   <i class="fas fa-thumbs-up" v-show="showLikeByIndex.index === index"></i>
                  </span>
                        ...
                        <div class="text-white d-inline ml-2 float-right" v-if="childData.user.name === user.data.attributes.name" >
                          <button class="btn-save mr-1"
                          >
                            <i class="fal fa-pen text-muted"  style="  color:#00BC7E !important;"
                               data-toggle="collapse" :href="'#multiCollapseExample_'+index" role="button"
                               aria-expanded="false" aria-controls="multiCollapseExample1"
                               @click.prevent="editComments(childData,index); showChildEditByIndex=index"></i>
                          </button>
                          <!---delete btn -->
                          <button class="btn-save"
                                  @click.prevent="updateComments(childData,'','delete'); childData.visible =!childData.visible">
                            <i class="fal fa-trash text-muted" style=" color:#00BC7E !important;"></i>
                          </button>
                        </div>
                      </div>
                      <!--    comment-->
                      <div class="pt-2" v-show="showChildEditByIndex ===index" >
                        <div class="d-inline">
                          <input type="text" class="form-control" v-model="edit_comment"  >
                        </div>
                        <button class="btn-save" style="color: #6c757d!important;"  @click.prevent="updateComments(childData,'','update'); showChildEditByIndex=''">Save</button>
                        <button class="btn-save" style="color: #6c757d!important;" @click.prevent="showChildEditByIndex ='';">Cancel</button>
                      </div>
                      <!--              v-if="isReplying"-->
                      <div class="pt-2"    v-show="showChildReplyByIndex ===childData.id && childData.parent_id ==data.id"  >
                        <div class="d-inline">
                          <input type="text" class="form-control" v-model="reply" >
                        </div>
                        <button class="btn-save" style="color: #6c757d!important;"  @click.prevent="replyComment(childData); showChildReplyByIndex ==='' ">Send</button>
                        <button class="btn-save" style="color: #6c757d!important;" @click.prevent="showChildReplyByIndex =''">Cancel</button>
                      </div>
                    </div>
                  </div>
                </div>
            ...
            <div class="row" v-else>
              <div class="col-sm-12">
                <table class="table table-sm" style="border:none;">
                  <tr>
                    <td class="text-muted" style="  color:#00BC7E !important;">File Name</td>
                    <td class="text" style="  color:#00BC7E !important;">  {{ fileName}}</td>
                  </tr>
                  <tr>
                    <td class="text-muted" style="  color:#00BC7E !important;">Uploader</td>
                    <td class="text" style="  color:#00BC7E !important;" >{{this.user.data.attributes.name}}</td>
                  </tr>
                  <tr>
                    <td class="text-muted"  style="  color:#00BC7E !important;">Upload Date</td>
                    <td class="text"  style="  color:#00BC7E !important;">{{this.currentFile.created_at}}</td>
                  </tr>
                  <tr>
                    <td class="text-muted" style="  color:#00BC7E !important;">Codec</td>
                    <td class="text"  style="  color:#00BC7E !important;">{{this.currentFile.mimetype}}</td>
                  </tr>
                  <tr>
                    <td class="text-muted" style="  color:#00BC7E !important;">RES</td>
                    <td class="text"  style="  color:#00BC7E !important;">1920 x 1080</td>
                  </tr>
                  <tr>
                    <td class="text-muted"  style="  color:#00BC7E !important;">FPS</td>
                    <td class="text" style="  color:#00BC7E !important;">{{this.currentFile.fps}}</td>
                  </tr>
                  <tr>
                    <td class="text-muted" style="  color:#00BC7E !important;">Audio</td>
                    <td class="text" style="  color:#00BC7E !important;">ACC</td>
                  </tr>
                  <tr>
                    <td class="text-muted" style="  color:#00BC7E !important;">Size</td>
                    <td class="text" style="  color:#00BC7E !important;">{{this.currentFile.filesize}}</td>
                  </tr>
                  <tr>
                    <td class="text-muted" style="  color:#00BC7E !important;">TC in</td>
                    <td class="text" style="  color:#00BC7E !important;">00:00:00:00</td>
                  </tr>
                  <tr>
                    <td class="text-muted" style="  color:#00BC7E !important;">TC Out</td>
                    <td class="text" style="  color:#00BC7E !important;">00:00:53:20</td>
                  </tr>
                  <tr>
                    <td class="text-muted" style="  color:#00BC7E !important;">Frames</td>
                    <td class="text" style="  color:#00BC7E !important;">{{getTotalFrame}}</td>
                  </tr>
                  <tr>
                    <td class="text-muted" style="  color:#00BC7E !important;">Date</td>
...
export default {
  name: 'SharedReviewPage',
  components: { ToolbarButton,Editor ,Spinner, VueSlider,Painting, FrameVideo,EmojiPicker,Tabs ,VueContentLoading,moment,
    FileFullPreview,
    DesktopToolbar,
  ...
  computed: {
    ...mapGetters(['myFileInfo','isLoggedIn','config','user','annotation','isProcessingFile','updateFps', 'data','isLogged','allComments','sharedFileID']),

    // myFileInfo: {
    //   get () {
    //     return this.$store.state.myFileInfo
    //   },
    //   set (value) {
    //     this.$store.commit('LOAD_FILEINFO_DETAIL', value)
    //   }
    // },
    ...
    resultQuery(){
      ...
      if(this.searchQuery){
        return this.allComments.filter(data => data.comment.toLowerCase().includes(this.searchQuery.toLowerCase()));
      }else{
  ...
  async beforeCreate() {
    ...
    {
      this.$router.push({path: '/sign-in?to='+this.$route.params.token, query: { q: this.$route.query.q }})
    }else{
      let vm =this
      // console.log('q',this.$route.query)
      // console.log('this.sharedFileID',this.sharedFileID)

      let q = this.$route.query ? atob(this.$route.query.q) : this.sharedFileID
      let route = `/api/files/${q}`
      await  axios
          .get(route)
          .then(response => {
            // End loading
            vm.$store.commit('LOAD_MY_FILE',response.data.data)
  ...
  updated(){
  //console.log('paint',this.$refs.paintable )
...
//  this.media.height = "500";
    ...
    let vm = this
    jQuery('#videoElement').on('click',function (){
  ...
  methods: {
    clearPaintable(){
      ...
      this.$refs.paintable.clearCanvas('')
      // this.$refs.paintable.canvasIsEmpty = true;
      // this.$refs.paintable.startedDrawing = true
    ...
    getSharedFiles() {
      ...
      //to the shared  the file with the current User
      if(this.currentFile.id) {
        axios
            .post('/api/share-complete-folder', {
              type: this.currentFile.type,
              isPassword: 0,
              unique_id: this.currentFile.unique_id
            })
            .then((response) => {
    ...
    getImage:  function() {
      this.$store.dispatch('getImage',
         {file_id:this.currentFile.id, user_id:this.user.data.id,currentTime:this.media.currentTime})
    },
    ...
    getComments() {
      ...
      this.$refs.paintable.ctx.clearRect(0, 0,  this.$refs.paintable.canvas.width,  this.$refs.paintable.canvas.height);
    ...
    submitComment:function () {
      ...
      this.isActive =='true' ? this.isActive =false : this.isActive =false
