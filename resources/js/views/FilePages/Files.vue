...
export default {
  ...
  computed: {
    ...mapGetters(['user', 'homeDirectory','isLogged', 'currentFolder','isLoggedIn', 'config']),
    favourites() {
  ...
  methods: {
    ...
    makeMember() {
      ...
      {
        axios
            .post('/api/checkMember',{user_id:this.user.data.id})
            .then(response =>{
              if(response.data.status === 400)
                axios
                    .post('/api/shareInvite',{team_id:this.$route.params.id,user_id:this.user.data.id,shareOptions:this.shareOptions})
                    .then(response => {
  ...
  mounted() {
    ...
    $('#content_file').css('position','none');
  }
}
</script>
