...
/* harmony default export */ __webpack_exports__["default"] = ({
  ...
  computed: _objectSpread({}, Object(vuex__WEBPACK_IMPORTED_MODULE_6__["mapGetters"])(['config', 'redirectUrl', 'isLoggedIn'])),
  data: function data() {
    return {
      isLoading: false,
            ...
            switch (_context2.prev = _context2.next) {
              ...
              case 5:
                ...
                axios__WEBPACK_IMPORTED_MODULE_8___default.a.post('/api/user/login', {
                  ...
                  _this2.$store.commit('SET_AUTHORIZED', true);

                  _this2.$store.commit('RETRIEVE_USER', response.data);

                  localStorage.setItem('auth_check', "true");

                  if (_this2.redirectUrl) {
                    _this2.$router.push({
                      path: _this2.redirectUrl
                    });
                  } else if (_this2.$route.query.to) {
                    window.location.href = 'sharedReview/' + _this2.shareRedirectUrl + '?q=' + _this2.$route.query.q; // this.$router.push({
                    //   path: '/sharedReview/'+this.shareRedirectUrl ,query:{ q:this.$route.query.q}
                    // })
                  } else {
                    // Go to files page
                    _this2.$router.push({
  ...
  created: function created() {
    ...
    if (this.$route.query.user_email) {
      this.loginEmail = this.$route.query.user_email;
      axios__WEBPACK_IMPORTED_MODULE_8___default.a.post('/api/user/check', {
        email: this.loginEmail
      }).then(function (response) {
        // End loading
        _this3.isLoading = false;
        _this3.checkedAccount = response.data; // Show sign in password page

        _this3.goToAuthPage('sign-in');
      })["catch"](function (error) {
        if (error.response.status == 404) {
          _this3.$refs.log_in.setErrors({
            'E-Mail': [error.response.data.message]
          });

          window.location.href = '/sign-up?user_email=' + _this3.loginEmail;
        ...
        if (error.response.status == 500) {
          _bus__WEBPACK_IMPORTED_MODULE_7__["events"].$emit('alert:open', {
            emoji: '🤔',
            title: _this3.$t('popup_signup_error.title'),
            message: _this3.$t('popup_signup_error.message')
          });
        }
      });
    }
    ...
    if (this.$route.query.to) {
      this.shareRedirectUrl = this.$route.query.to;
    }
  }
});
